...
PaddleOCR_v35::PaddleOCR_v35(const PaddleOCRParams &params) : params_(params) {
...
PaddleOCR_v35::Predict(const std::vector<std::string> &input) {
  return pipeline_infer_->Predict(input);
}
void PaddleOCR_v35::CreatePipeline() {
  pipeline_infer_ = std::unique_ptr<BasePipeline>(
      new OCRPipeline(ToOCRPipelineParams(params_)));
}

absl::Status PaddleOCR_v35::CheckParams() {
...
OCRPipelineParams PaddleOCR_v35::ToOCRPipelineParams(const PaddleOCRParams &from) {
